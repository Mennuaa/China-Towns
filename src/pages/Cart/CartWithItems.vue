<script setup>
const array = ['']
</script>

<template>
	<div :class="['cart', { 'cart_mobile': !isLargeScreen }]" id="cart">
		<div class="container">
			<div class="cart-section">
			<div class="cart_left">
                <ProfileLinks />
            </div>
				<div v-show="array.length > 0" class="cart_right">
                    <!-- <div @click="goBack()" class="profile_mobile_go_back">
                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M9.33252 10.6672L6.66585 8.00057L9.33252 5.3339" stroke="#323232"
                                stroke-linecap="round" />
                        </svg>
                        <span>Назад</span>
                    </div> -->
					<h2 class="cart__title">Корзина</h2>
					<p class="cart__subtitle">
						Здесь содержатся товары добавляемые с помощью расширения
						<a class="cart__link link-style" href="#"> WAIX Extension </a>
					</p>
					<div class="cart-info">
						<div class="cart-info__header">
							<p>Товар</p>
							<p>Кол-во</p>
							<p>Сумма</p>
							<p>Доставка</p>
							<p>Услуги</p>
							<p>Итого</p>
						</div>

						<div class="cart-info__product">
							<div class="cart-info__product-content">
								<input type="checkbox" />
								<img src="../../../public/images/product.png" alt="cartProductImg" />
								<div class="cart-info__product-text">
									<p>
										义乌市胜美化妆品有限公司实力供应商
										<a href="#">
											花基品西梅红茶茉莉白柚柠檬白茶女士学生自然持久淡香水50ml批发
										</a>
									</p>
									<div class="cart-info__product-subtext">
										<p>颜色分类 <span>HJP005西梅红茶</span></p>
										<p>净含量 <span>50ml</span></p>
									</div>
								</div>
							</div>
							<div class="cart-info__product-rigth">
								<div class="cart-info__product-rigth__amount">
									<p class="text__typography-sm">Кол-во</p>
									<div class="cart-info__product-text__grid">
										<Counter />
									</div>
								</div>

								<div class="">
									<p class="text__typography-sm">Сумма</p>
									<div class="cart-info__product-text__grid">
										<p class="text__typography-sm__black">¥ 6.00</p>
										<p class="text__typography-sm__black">¥ 6.00</p>
									</div>
								</div>
								<div class="">
									<p class="text__typography-sm">Доставка</p>
									<div class="cart-info__product-text__grid">
										<p class="text__typography-sm__black">¥ 6.00</p>
										<p class="text__typography-sm__black">¥ 6.00</p>
									</div>
								</div>
								<div class="">
									<p class="text__typography-sm">Услуги</p>
									<div class="cart-info__product-text__grid">
										<p class="text__typography-sm__black">¥ 6.00</p>
										<p class="text__typography-sm__black">¥ 6.00</p>
									</div>
								</div>
								<div class="">
									<p class="text__typography-sm">Итого</p>
									<div class="cart-info__product-text__grid">
										<p class="text__typography-sm__black">¥ 6.00</p>
										<p class="text__typography-sm__black">¥ 6.00</p>
									</div>
								</div>
							</div>

							<svg
								class="cart-info__delete"
								width="16"
								height="16"
								viewBox="0 0 16 16"
								fill="none"
								xmlns="http://www.w3.org/2000/svg"
							>
								<path
									d="M13.5 3H10.5V2.25C10.5 1.91848 10.3683 1.60054 10.1339 1.36612C9.89946 1.1317 9.58152 1 9.25 1H6.75C6.41848 1 6.10054 1.1317 5.86612 1.36612C5.6317 1.60054 5.5 1.91848 5.5 2.25V3H2.5C2.36739 3 2.24021 3.05268 2.14645 3.14645C2.05268 3.24021 2 3.36739 2 3.5C2 3.63261 2.05268 3.75979 2.14645 3.85355C2.24021 3.94732 2.36739 4 2.5 4H3.03125L3.625 13.5288C3.66937 14.3678 4.3125 15 5.125 15H10.875C11.6916 15 12.3219 14.3819 12.375 13.5312L12.9688 4H13.5C13.6326 4 13.7598 3.94732 13.8536 3.85355C13.9473 3.75979 14 3.63261 14 3.5C14 3.36739 13.9473 3.24021 13.8536 3.14645C13.7598 3.05268 13.6326 3 13.5 3ZM6.01781 13H6C5.87042 13.0001 5.74587 12.9499 5.6526 12.8599C5.55933 12.77 5.50462 12.6473 5.5 12.5178L5.25 5.51781C5.24528 5.3852 5.29342 5.25615 5.38385 5.15904C5.47428 5.06193 5.59958 5.00472 5.73219 5C5.8648 4.99528 5.99385 5.04342 6.09096 5.13385C6.18807 5.22428 6.24528 5.34958 6.25 5.48219L6.5 12.4822C6.50238 12.5479 6.49179 12.6134 6.46884 12.6749C6.44589 12.7365 6.41103 12.793 6.36624 12.841C6.32146 12.8891 6.26764 12.9279 6.20785 12.9552C6.14806 12.9825 6.08349 12.9977 6.01781 13ZM8.5 12.5C8.5 12.6326 8.44732 12.7598 8.35355 12.8536C8.25979 12.9473 8.13261 13 8 13C7.86739 13 7.74021 12.9473 7.64645 12.8536C7.55268 12.7598 7.5 12.6326 7.5 12.5V5.5C7.5 5.36739 7.55268 5.24021 7.64645 5.14645C7.74021 5.05268 7.86739 5 8 5C8.13261 5 8.25979 5.05268 8.35355 5.14645C8.44732 5.24021 8.5 5.36739 8.5 5.5V12.5ZM9.5 3H6.5V2.25C6.49962 2.21706 6.50583 2.18439 6.51826 2.15388C6.53069 2.12338 6.54909 2.09567 6.57238 2.07238C6.59567 2.04909 6.62338 2.03069 6.65388 2.01826C6.68439 2.00583 6.71706 1.99962 6.75 2H9.25C9.28294 1.99962 9.31561 2.00583 9.34612 2.01826C9.37662 2.03069 9.40433 2.04909 9.42762 2.07238C9.45091 2.09567 9.46931 2.12338 9.48174 2.15388C9.49417 2.18439 9.50038 2.21706 9.5 2.25V3ZM10.5 12.5178C10.4954 12.6473 10.4407 12.77 10.3474 12.8599C10.2541 12.9499 10.1296 13.0001 10 13H9.98188C9.91623 12.9977 9.85168 12.9824 9.79193 12.9551C9.73218 12.9278 9.67839 12.889 9.63364 12.8409C9.58889 12.7929 9.55406 12.7364 9.53113 12.6749C9.5082 12.6133 9.49762 12.5478 9.5 12.4822L9.75 5.48219C9.75234 5.41653 9.76759 5.35197 9.79488 5.2922C9.82217 5.23243 9.86096 5.17863 9.90904 5.13385C9.95713 5.08908 10.0136 5.05421 10.0751 5.03124C10.1367 5.00828 10.2022 4.99766 10.2678 5C10.3335 5.00234 10.398 5.01759 10.4578 5.04488C10.5176 5.07217 10.5714 5.11096 10.6161 5.15904C10.6609 5.20713 10.6958 5.26356 10.7188 5.32511C10.7417 5.38667 10.7523 5.45215 10.75 5.51781L10.5 12.5178Z"
									fill="#191617"
								/>
							</svg>
						</div>
						<hr />
						<div class="cart-info__product">
							<div class="cart-info__product-content">
								<input type="checkbox" />
								<img src="../../../public/images/product.png" alt="cartProductImg" />
                                
								<div class="cart-info__product-text">
									<p class="cart-info__product-text-p">
										花基品西梅红茶茉莉白柚柠檬白茶女士学生自然持久淡香水50ml批发
										<!-- <a href="#">
											花基品西梅红茶茉莉白柚柠檬白茶女士学生自然持久淡香水50ml批发
										</a> -->
									</p>
									<div class="cart-info__product-subtext">
										<p>颜色分类 <span>HJP005西梅红茶</span></p>
										<p>净含量 <span>50ml</span></p>
									</div>
								</div>
							</div>
							<div class="cart-info__product-rigth">
								<div class="cart-info__product-rigth__amount">
									<p class="text__typography-sm">Кол-во</p>
									<div class="cart-info__product-text__grid">
										<Counter />
									</div>
								</div>

								<div class="">
									<p class="text__typography-sm">Сумма</p>
									<div class="cart-info__product-text__grid">
										<p class="text__typography-sm__black">¥ 6.00</p>
										<p class="text__typography-sm__black">¥ 6.00</p>
									</div>
								</div>
								<div class="">
									<p class="text__typography-sm">Доставка</p>
									<div class="cart-info__product-text__grid">
										<p class="text__typography-sm__black">¥ 6.00</p>
										<p class="text__typography-sm__black">¥ 6.00</p>
									</div>
								</div>
								<div class="">
									<p class="text__typography-sm">Услуги</p>
									<div class="cart-info__product-text__grid">
										<p class="text__typography-sm__black">¥ 6.00</p>
										<p class="text__typography-sm__black">¥ 6.00</p>
									</div>
								</div>
								<div class="">
									<p class="text__typography-sm">Итого</p>
									<div class="cart-info__product-text__grid">
										<p class="text__typography-sm__black">¥ 6.00</p>
										<p class="text__typography-sm__black">¥ 6.00</p>
									</div>
								</div>
							</div>

							<svg
								class="cart-info__delete"
								width="16"
								height="16"
								viewBox="0 0 16 16"
								fill="none"
								xmlns="http://www.w3.org/2000/svg"
							>
								<path
									d="M13.5 3H10.5V2.25C10.5 1.91848 10.3683 1.60054 10.1339 1.36612C9.89946 1.1317 9.58152 1 9.25 1H6.75C6.41848 1 6.10054 1.1317 5.86612 1.36612C5.6317 1.60054 5.5 1.91848 5.5 2.25V3H2.5C2.36739 3 2.24021 3.05268 2.14645 3.14645C2.05268 3.24021 2 3.36739 2 3.5C2 3.63261 2.05268 3.75979 2.14645 3.85355C2.24021 3.94732 2.36739 4 2.5 4H3.03125L3.625 13.5288C3.66937 14.3678 4.3125 15 5.125 15H10.875C11.6916 15 12.3219 14.3819 12.375 13.5312L12.9688 4H13.5C13.6326 4 13.7598 3.94732 13.8536 3.85355C13.9473 3.75979 14 3.63261 14 3.5C14 3.36739 13.9473 3.24021 13.8536 3.14645C13.7598 3.05268 13.6326 3 13.5 3ZM6.01781 13H6C5.87042 13.0001 5.74587 12.9499 5.6526 12.8599C5.55933 12.77 5.50462 12.6473 5.5 12.5178L5.25 5.51781C5.24528 5.3852 5.29342 5.25615 5.38385 5.15904C5.47428 5.06193 5.59958 5.00472 5.73219 5C5.8648 4.99528 5.99385 5.04342 6.09096 5.13385C6.18807 5.22428 6.24528 5.34958 6.25 5.48219L6.5 12.4822C6.50238 12.5479 6.49179 12.6134 6.46884 12.6749C6.44589 12.7365 6.41103 12.793 6.36624 12.841C6.32146 12.8891 6.26764 12.9279 6.20785 12.9552C6.14806 12.9825 6.08349 12.9977 6.01781 13ZM8.5 12.5C8.5 12.6326 8.44732 12.7598 8.35355 12.8536C8.25979 12.9473 8.13261 13 8 13C7.86739 13 7.74021 12.9473 7.64645 12.8536C7.55268 12.7598 7.5 12.6326 7.5 12.5V5.5C7.5 5.36739 7.55268 5.24021 7.64645 5.14645C7.74021 5.05268 7.86739 5 8 5C8.13261 5 8.25979 5.05268 8.35355 5.14645C8.44732 5.24021 8.5 5.36739 8.5 5.5V12.5ZM9.5 3H6.5V2.25C6.49962 2.21706 6.50583 2.18439 6.51826 2.15388C6.53069 2.12338 6.54909 2.09567 6.57238 2.07238C6.59567 2.04909 6.62338 2.03069 6.65388 2.01826C6.68439 2.00583 6.71706 1.99962 6.75 2H9.25C9.28294 1.99962 9.31561 2.00583 9.34612 2.01826C9.37662 2.03069 9.40433 2.04909 9.42762 2.07238C9.45091 2.09567 9.46931 2.12338 9.48174 2.15388C9.49417 2.18439 9.50038 2.21706 9.5 2.25V3ZM10.5 12.5178C10.4954 12.6473 10.4407 12.77 10.3474 12.8599C10.2541 12.9499 10.1296 13.0001 10 13H9.98188C9.91623 12.9977 9.85168 12.9824 9.79193 12.9551C9.73218 12.9278 9.67839 12.889 9.63364 12.8409C9.58889 12.7929 9.55406 12.7364 9.53113 12.6749C9.5082 12.6133 9.49762 12.5478 9.5 12.4822L9.75 5.48219C9.75234 5.41653 9.76759 5.35197 9.79488 5.2922C9.82217 5.23243 9.86096 5.17863 9.90904 5.13385C9.95713 5.08908 10.0136 5.05421 10.0751 5.03124C10.1367 5.00828 10.2022 4.99766 10.2678 5C10.3335 5.00234 10.398 5.01759 10.4578 5.04488C10.5176 5.07217 10.5714 5.11096 10.6161 5.15904C10.6609 5.20713 10.6958 5.26356 10.7188 5.32511C10.7417 5.38667 10.7523 5.45215 10.75 5.51781L10.5 12.5178Z"
									fill="#191617"
								/>
							</svg>
						</div>
					</div>
					<div class="cart-btns">
						<a href="#" class="cart-btns__link disabled"
							>Создать заказ из выбранных товаров</a
						>
						<a href="#" class="cart-btns__link"
							>Создать заказ из всех товаров</a
						>
					</div>
				</div>
			</div>
		</div>
	</div>
</template>
<script>
import Counter from '@/components/Counter.vue';
import ProfileLinks from '@/components/ProfileLinks.vue';

export default {
    name: 'CartWithItems',
    components: {
        Counter,
        ProfileLinks
    },
    data() {
        return {
            count: 1,
            isLargeScreen: window.innerWidth > 1050,
            showPopupVideo: false,
        
        };
    },
    methods: {
        incCount() {
            this.count++;
        },
        decCount() {
            this.count--;
        },
        deleteFromCart() {
            this.count = 0;
        }
    }
}
</script>
<style>
/* @media screen and (max-width: 1050px) {
    .cart_left{
        display: none;
    }
    .cart_right{
        width: 100% !important;
    }
} */
</style>


<style src="./cart-w-i.css"></style>

